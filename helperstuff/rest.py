...
class McM(object):
    def __init__(self, id='sso', debug=False, cookie=None, dev=True, int=False, cookie_on_demand=False):
        self.dev = dev
        if self.dev:
        ...
        self.__cookie_on_demand = cookie_on_demand

    def __newcookie(self):
        os.system('eval $(scram unsetenv -sh) && cern-get-sso-cookie -u https://%s -o %s --krb'
                  % (self.server, self.cookie_filename))
        if not os.path.isfile(self.cookie_filename):
            raise RuntimeError("Failed to make the sso cookie file")
    ...
    def __get(self, url, _tryagain=True):
    ...
    def __put(self, url, data, _tryagain=True):
        if self.id != 'sso' and self.id != 'cert':
            url = '/mcm/' + url
