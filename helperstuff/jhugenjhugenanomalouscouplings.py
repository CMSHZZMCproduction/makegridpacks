...
#    print folder
    ...
    if self.year in (2016, 2017, 2018) and cardbase == "ggZH" and self.coupling == "SM":
...
#    return False
  ...
  def tarballversion(self):
    v = 1
    if self.productionmode == "ggZH": v +=1
    return v
  ...
  def cvmfstarball_anyversion(self, version):
    ...
    if self.year in (2016, 2017, 2018):
      folder = os.path.join("/cvmfs/cms.cern.ch/phys_generator/gridpacks/2017/13TeV/jhugen/V727", self.productionmode+"_ZZ_NNPDF31_13TeV")

    #tarballname = self.datasetname+".tgz"
    tarballname = self.datasetname.replace("_TuneCP5", "")+".tgz"
...
#    return super(JHUGenJHUGenAnomCoupMCSample, self).genproductionscommitforfragment
  ...
  def maxallowedtimeperevent(self):
    ...
    if self.productionmode == "ggZH": return 500
  ...
  @property
  def nthreads(self):
    if self.productionmode == "ggZH" and self.coupling == "L1Zgmix": return 16
    return super(JHUGenJHUGenAnomCoupMCSample, self).nthreads

  @property
  def memory(self):
    if self.productionmode == "ggZH" and self.coupling == "L1Zgmix": return 15900
    if self.nthreads == 1: return 2300
    return super(JHUGenJHUGenAnomCoupMCSample, self).memory

  def handle_request_fragment_check_warning(self, line):
    if self.productionmode == "ggZH" and line == "* [WARNING] Do you really want to have tune CP5 in this campaign?":
      return "ok"
    return super(JHUGenJHUGenAnomCoupMCSample, self).handle_request_fragment_check_warning(line)
